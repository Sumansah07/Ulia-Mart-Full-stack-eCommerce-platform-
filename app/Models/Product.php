...
class Product extends Model
{
     ...
     * First check if product has SKU, otherwise get from first variation
     */
    public function getSku()
    {
        // If product has SKU directly, return it
        if (!empty($this->sku)) {
            return $this->sku;
        }

        // If product has code directly, return it
        if (!empty($this->code)) {
            return $this->code;
        }

        // Otherwise, get SKU from first variation
        $firstVariation = $this->variations()->first();
        if ($firstVariation) {
            return $firstVariation->sku ?: $firstVariation->code;
        }

        // Return empty if no SKU found
        return '';
    }

    /**
     * Get the product code
     * First check if product has code, otherwise get from first variation
     */
    public function getCode()
    {
        ...
        // Otherwise, get code from first variation
        $firstVariation = $this->variations()->first();
        if ($firstVariation) {
            return $firstVariation->code ?: $firstVariation->sku;
        }
