...
class OrderController extends Controller
{
    ...
    public function summery(Request $request)
    {
        ...
        return response()->json([
            ...
            'tax'      => formatPrice(getTotalTaxWithCouponDiscount($carts, getCoupon())),
            'shipping_charge' => formatPrice($shippingAmount),
            'is_free_shipping' => $is_free_shipping,
            'coupon_discount' => formatPrice(getCouponDiscountWithRestrictions($carts, getCoupon())),
            'total' => formatPrice(getSubTotal($carts, false, '', false) + getTotalTaxWithCouponDiscount($carts, getCoupon()) + $shipping - getCouponDiscountWithRestrictions($carts, getCoupon())),
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if (count($carts) > 0) {
            ...
            $orderGroup->total_tax_amount                   = getTotalTaxWithCouponDiscount($carts, getCoupon());
