...
class DeliveryOrdersController extends Controller
{
    ...
    public function updateDeliveryStatus(Request $request)
    {
        ...
        // Send email notifications to customer if status changed
        if ($oldDeliveryStatus !== $order->delivery_status) {
            $this->sendDeliveryStatusEmail($order, $oldDeliveryStatus, $order->delivery_status);
        }

        if ($oldPaymentStatus !== $order->payment_status) {
            $this->sendPaymentStatusEmail($order, $oldPaymentStatus, $order->payment_status);
        }
    ...
    function cancelOrder(Request $request, Order $order){
        ...
        if ($oldStatus !== 'cancelled') {
            $this->sendDeliveryStatusEmail($order, $oldStatus, 'cancelled');
        }
