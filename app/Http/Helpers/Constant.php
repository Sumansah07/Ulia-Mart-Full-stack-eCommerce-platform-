<?php

use App\Models\Coupon;
use App\Models\Variation;
use App\Models\CouponUsage;

use App\Models\EnmartModule;
use App\Models\Localization;
use App\Models\MediaManager;
use Illuminate\Http\Request;
use App\Models\SystemSetting;
use App\Models\VariationValue;
use Nwidart\Modules\Facades\Module;
use Illuminate\Support\Facades\Config;
use Lunaweb\RecaptchaV3\Facades\RecaptchaV3;
use App\Http\Middleware\ApiCurrencyMiddleWare;
use App\Models\Category;
use App\Models\Product;
use App\Models\ProductTheme;
use App\Models\Theme;
use Illuminate\Support\Facades\Schema;
use League\CommonMark\Normalizer\SlugNormalizer;
use Illuminate\Support\Facades\Artisan;



use App\Http\Helpers\ImageHelper;

if (!function_exists('ddError')) {
    # get error  information
    function ddError($e)
    {
        return dd(errorArray($e));
    }
}


if (!function_exists('errorArray')) {
    # get error  information
    function errorArray($e)
    {
        return [
            "message" => $e->getMessage(),
            "line" => $e->getLine(),
            "file" => $e->getFile(),
        ];
    }
}

if (!function_exists('user')) {
    # get user information
    function user()
    {
        return auth()->user();
    }
}


if (!function_exists('getMyShopId')) {
    # get user shop id
    function getMyShopId()
    {
        return auth()->user()->shop_id ?? 1;
    }
}


if (!function_exists('userID')) {
    # get user id
    function userID()
    {
        return auth()->id();
    }
}


# Default Image use as No Image.
if (!function_exists('noImage')) {
    function noImage()
    {
        return asset("public/frontend/default/assets/img/no-data-found.png");
    }
}

if (!function_exists('isAdmin')) {
    # get is admin
    function isAdmin()
    {
        return user()->user_type == 'admin';
    }
}

# Is Customer
if (!function_exists('isCustomer')) {
    function isCustomer()
    {
        return user()->user_type == "customer";
    }
}



if (!function_exists('isPublished')) {
    # get is published
    function isPublished($value = 1)
    {
        return $value != 0;
    }
}


if (!function_exists('isLoggedIn')) {
    # get is Logged in
    function isLoggedIn()
    {
        return auth()->check();
    }
}

if (!function_exists('getTheme')) {
    # get system theme
    function getTheme()
    {
        if (session('theme') != null && session('theme') != '') {
            return session('theme');
        }
        return Config::get('app.theme');
    }
}

if (!function_exists('getView')) {
    # get view of theme
    function getView($path, $data = [])
    {
        return view('frontend.' . getTheme() . '.' . $path, $data);
    }
}

if (!function_exists('getViewRender')) {
    # get view of theme with render
    function getViewRender($path, $data = [])
    {
        return view('frontend.' . getTheme() . '.' . $path, $data)->render();
    }
}

if (!function_exists('getRender')) {
    # get view of theme with render
    function getRender($path, $data = [])
    {
        return view('frontend.default'. '.' . $path, $data)->render();
    }
}

if (!function_exists('cacheClear')) {
    # clear server cache
    function cacheClear()
    {
        try {
            Artisan::call('cache:forget spatie.permission.cache');
        } catch (\Throwable $th) {
            //throw $th;
        }

        Artisan::call('cache:clear');
        Artisan::call('view:clear');
        Artisan::call('config:clear');
    }
}

if (!function_exists('clearOrderSession')) {
    # clear session cache
    function clearOrderSession()
    {
        session()->forget('payment_method');
        session()->forget('payment_type');
        session()->forget('order_code');
        session()->forget('pending_order_data');
        session()->forget('payment_amount');
        session()->forget('fonepay_reference_id');
        session()->forget('imepay_reference_id');
    }
}

if (!function_exists('csrfToken')) {
    #  Get the CSRF token value.
    function csrfToken()
    {
        $session = app('session');

        if (isset($session)) {
            return $session->token();
        }
        throw new RuntimeException('Session store not set.');
    }
}

if (!function_exists('paginationNumber')) {
    # return number of data per page
    function paginationNumber($value = null)
    {
        $number = is_int(getSetting('pagination')) && getSetting('pagination') > 9 ? getSetting('pagination') : env('DEFAULT_PAGINATION');
        return $value != null ? $value : $number;
    }
}

if (!function_exists('areActiveRoutes')) {
    # return active class
    function areActiveRoutes(array $routes, $output = "active")
    {
        foreach ($routes as $route) {
            if (Route::currentRouteName() == $route) return $output;
        }
        return '';
    }
}

if (!function_exists('validatePhone')) {
    # validatePhone
    function validatePhone($phone)
    {
        $phone = str_replace(' ', '', $phone);
        $phone = str_replace('-', '', $phone);
        return $phone;
    }
}


if (!function_exists('staticAsset')) {
    # return path for static assets
    function staticAsset($path, $secure = null)
    {
        $isLocalEnvironment = str_contains(url('/'), '.test') || str_contains(url('/'), 'http://127.0.0.1:') || str_contains(url('/'), 'localhost');

        // Determine the file path based on environment
        $filePath = $isLocalEnvironment ? $path : 'public/' . $path;

        // Check if the file exists
        if (!file_exists($filePath) && !str_starts_with($path, 'http')) {
            // Create a placeholder for missing files
            $dirPath = dirname($filePath);
            if (!is_dir($dirPath)) {
                mkdir($dirPath, 0777, true);
            }

            // Create a simple placeholder file
            $content = "Placeholder for static asset";
            file_put_contents($filePath, $content);
        }

        // Return the asset URL based on environment
        return $isLocalEnvironment ?
            app('url')->asset($path, $secure) . '?v=' . env('APP_VERSION') :
            app('url')->asset('public/' . $path, $secure) . '?v=' . env('APP_VERSION');
    }
}

if (!function_exists('uploadedAsset')) {
    #  Generate an asset path for the uploaded files.
    function uploadedAsset($fileId)
    {
        $mediaFile = MediaManager::find($fileId);
        if (!is_null($mediaFile)) {
            $isLocalEnvironment = str_contains(url('/'), '.test') || str_contains(url('/'), 'http://127.0.0.1:') || str_contains(url('/'), 'localhost');

            // Determine the file path based on environment
            $filePath = $isLocalEnvironment ? $mediaFile->media_file : 'public/' . $mediaFile->media_file;

            // Check if the file exists
            if (!file_exists($filePath) && !str_starts_with($mediaFile->media_file, 'http')) {
                // Create a placeholder for missing files
                $dirPath = dirname($filePath);
                if (!is_dir($dirPath)) {
                    mkdir($dirPath, 0777, true);
                }

                // Create a simple placeholder file
                $content = "Placeholder for media ID $fileId";
                file_put_contents($filePath, $content);

                // Update the media size
                $mediaFile->media_size = filesize($filePath);
                $mediaFile->save();
            }

            // Return the asset URL based on environment
            return $isLocalEnvironment ?
                app('url')->asset($mediaFile->media_file) :
                app('url')->asset('public/' . $mediaFile->media_file);
        }
        return noImage();
    }
}

if (!function_exists('uploadedAssetes')) {
    #  Generate an asset path for the uploaded files.
    function uploadedAssetes($ids)
    {
        $ids = explode(",", $ids);

        $assets = [];
        $mediaFiles = MediaManager::whereIn('id', $ids)->get();

        if ($mediaFiles) {
            foreach ($mediaFiles as $file) {
                if (str_contains(url('/'), '.test') || str_contains(url('/'), 'http://127.0.0.1:') || str_contains(url('/'), 'localhost')) {
                    $assets[] = app('url')->asset('' . $file->media_file);
                } else {
                    $assets[] = app('url')->asset('public/' . $file->media_file);
                }
            }
        }
        return $assets;
    }
}

if (!function_exists('productStock')) {
    #  get product stock
    function productStock($product)
    {

        $isVariantProduct = 0;
        $stock = 0;
        if ($product->variations()->count() > 1) {
            $isVariantProduct = 1;
        } else {
            $stock = $product->variations[0]->product_variation_stock ? $product->variations[0]->product_variation_stock->stock_qty : 0;
        }
        return $stock;
    }
}



if (!function_exists('localize')) {
    # add / return localization
    function localize($key, $lang = null)
    {
        if ($lang == null) {
            $lang = App::getLocale();
        }

        $t_key = preg_replace('/[^A-Za-z0-9\_]/', '', str_replace(' ', '_', strtolower($key)));

        $localization_english = Cache::rememberForever('localizations-en', function () {
            return Localization::where('lang_key', 'en')->pluck('t_value', 't_key');
        });

        if (!isset($localization_english[$t_key])) {
            # add new localization
            newLocalization('en', $t_key, $key);
        }

        # return user session lang
        $localization_user = Cache::rememberForever("localizations-{$lang}", function () use ($lang) {
            return Localization::where('lang_key', $lang)->pluck('t_value', 't_key')->toArray();
        });

        if (isset($localization_user[$t_key])) {
            return trim($localization_user[$t_key]);
        }

        return isset($localization_english[$t_key]) ? trim($localization_english[$t_key]) : $key;
    }
}

if (!function_exists('newLocalization')) {
    # new localization
    function newLocalization($lang, $t_key, $key)
    {
        $localization = new Localization;
        $localization->lang_key = $lang;
        $localization->t_key = $t_key;
        $localization->t_value = str_replace(array("\r", "\n", "\r\n"), "", $key);
        $localization->save();

        # clear cache
        Cache::forget('localizations-' . $lang);

        return trim($key);
    }
}

if (!function_exists('writeToEnvFile')) {
    # write To Env File
    function
    writeToEnvFile($type, $val)
    {
        if (env('DEMO_MODE') != 'On') {
            $path = base_path('.env');
            if (file_exists($path)) {
                $val = '"' . trim($val) . '"';
                if (is_numeric(strpos(file_get_contents($path), $type)) && strpos(file_get_contents($path), $type) >= 0) {
                    file_put_contents($path, str_replace(
                        $type . '="' . env($type) . '"',
                        $type . '=' . $val,
                        file_get_contents($path)
                    ));
                } else {
                    file_put_contents($path, file_get_contents($path) . "\r\n" . $type . '=' . $val);
                }
            }
        }
    }
}

if (!function_exists('getFileType')) {
    #  Get file Type
    function getFileType($type)
    {
        $fileTypeArray = [
            // audio
            "mp3"       =>  "audio",
            "wma"       =>  "audio",
            "aac"       =>  "audio",
            "wav"       =>  "audio",

            // video
            "mp4"       =>  "video",
            "mpg"       =>  "video",
            "mpeg"      =>  "video",
            "webm"      =>  "video",
            "ogg"       =>  "video",
            "avi"       =>  "video",
            "mov"       =>  "video",
            "flv"       =>  "video",
            "swf"       =>  "video",
            "mkv"       =>  "video",
            "wmv"       =>  "video",

            // image
            "png"       =>  "image",
            "svg"       =>  "image",
            "gif"       =>  "image",
            "jpg"       =>  "image",
            "jpeg"      =>  "image",
            "webp"      =>  "image",

            // document
            "doc"       =>  "document",
            "txt"       =>  "document",
            "docx"      =>  "document",
            "pdf"       =>  "document",
            "csv"       =>  "document",
            "xml"       =>  "document",
            "ods"       =>  "document",
            "xlr"       =>  "document",
            "xls"       =>  "document",
            "xlsx"      =>  "document",

            // archive
            "zip"       =>  "archive",
            "rar"       =>  "archive",
            "7z"        =>  "archive"
        ];
        return isset($fileTypeArray[$type]) ? $fileTypeArray[$type] : null;
    }
}

if (!function_exists('fileDelete')) {
    # file delete
    function fileDelete($file)
    {
        if (File::exists('public/' . $file)) {
            File::delete('public/' . $file);
        }
    }
}

if(!function_exists('isGreater')) {
    function isGreater($currentVersion, $upcomingVersion, $isNumberConversion = false, $replaceValue = ["."], $replaceWith = "")
    {

        if($isNumberConversion) {
            $currentVersion  = intval(str_replace($replaceValue, $replaceWith, $currentVersion));
            $upcomingVersion = intval(str_replace($replaceValue, $replaceWith, $upcomingVersion));
        }

        return $currentVersion < $upcomingVersion;
    }
}


if(!function_exists('getNumberFromString')) {
    function getNumberFromString($str, $replaceValue = ["."], $replaceWith = "")
    {
        return intval(str_replace($replaceValue, $replaceWith, $str));
    }
}


if(!function_exists('currentVersion')) {
    function currentVersion($isNumber = false)
    {
        # need to check bcz of setup route
        if(Schema::hasTable('system_settings')) {
            $settings = SystemSetting::where('entity', 'software_version')->first();
            if($settings) {
                $version = $settings->value;
            }
        }
        if(empty($version)){
            $version = env('APP_VERSION') ? str_replace('v', '', env('APP_VERSION')) : null;
        }

        return $isNumber ? intval(str_replace(".", "", $version)) : $version;
    }
}


if (!function_exists('getSetting')) {
    # return system settings value
    function getSetting($key, $default = null)
    {
        try {
            $settings = Cache::remember('settings', 86400, function () {
                return SystemSetting::all();
            });

            $setting = $settings->where('entity', $key)->first();

            return $setting == null ? $default : $setting->value;
        } catch (\Throwable $th) {
            return $default;
        }
    }
}

if (!function_exists('renderStarRating')) {
    # render ratings
    function renderStarRating($rating, $maxRating = 5)
    {
        $fullStar = "<i data-feather='star' width='16' height='16' class='text-primary'></i>";

        $rating = $rating <= $maxRating ? $rating : $maxRating;
        $fullStarCount = (int)$rating;

        $html = str_repeat($fullStar, $fullStarCount);
        echo $html;
    }
}

if (!function_exists('renderStarRatingFront')) {
    # render ratings frontend
    function renderStarRatingFront($rating, $maxRating = 5)
    {
        $fullStar = '<li><i class="fas fa-star"></i></li>';
        if(getTheme() == "halal"){
            $fullStar = '<li><span class="d-inline-block fs-14 clr-warning">
                <i class="fas fa-star"></i>
            </span></li>';
        }

        $rating = $rating <= $maxRating ? $rating : $maxRating;
        $fullStarCount = (int)$rating;

        $html = str_repeat($fullStar, $fullStarCount);
        echo $html;
    }
}

if (!function_exists('formatPrice')) {

    //formats price - truncate price to 1M, 2K if activated by admin
    function formatPrice($price, $truncate = false, $forceTruncate = false, $addSymbol = true, $numberFormat = true)

    {
        // No currency conversion - use the price as is

        if ($numberFormat) {
            // truncate price
            if ($truncate) {
                if (getSetting('truncate_price') == 1 || $forceTruncate == true) {
                    if ($price < 1000000) {
                        // less than a million
                        $price = number_format($price, getSetting('no_of_decimals'));
                    } else if ($price < 1000000000) {
                        // less than a billion
                        $price = number_format($price / 1000000, getSetting('no_of_decimals')) . 'M';
                    } else {
                        // at least a billion
                        $price = number_format($price / 1000000000, getSetting('no_of_decimals')) . 'B';
                    }
                }
            } else {
                // decimals
                if (getSetting('no_of_decimals') > 0) {
                    $price = number_format($price, getSetting('no_of_decimals'));
                } else {
                    $price = number_format($price, getSetting('no_of_decimals'), '.', ',');
                }
            }
        }

        if ($addSymbol) {
            // Always use Rs as the currency symbol with left alignment
            return 'Rs. ' . $price;
        }
        return $price;
    }
}

if (!function_exists('apiProductPrice')) {
    //formats price - truncate price to 1M, 2K if activated by admin
    function apiProductPrice($product)
    {
        $price = "00";
        if (productBasePrice($product) == discountedProductBasePrice($product)) {
            if (productBasePrice($product) == productMaxPrice($product)) {
                $price = formatPrice(productBasePrice($product));
            } else {
                $price = formatPrice(productBasePrice($product)) .
                    "-"
                    . formatPrice(productMaxPrice($product));
            }
        } else {
            if (discountedProductBasePrice($product) == discountedProductMaxPrice($product))
                $price = formatPrice(discountedProductBasePrice($product));
            else
                $price = formatPrice(discountedProductBasePrice($product)) .
                    "-"
                    . formatPrice(discountedProductMaxPrice($product));
        }
        return $price;
    }
}


if (!function_exists('priceToUsd')) {
    // price to usd
    function priceToUsd($price)
    {
        // convert amount equal to local currency
        if (Session::has('currency_code') && Session::has('local_currency_rate')) {
            $price = floatval($price) / floatval(Session::get('local_currency_rate'));
        }

        return $price;
    }
}

if (!function_exists('getValidCurrency')) {
    /**
     * Get a valid currency object, with fallback to USD
     *
     * @param string $code Currency code to retrieve
     * @return \App\Models\Currency
     */
    function getValidCurrency($code = null)
    {
        // Use provided code or get from session
        if (!$code) {
            $code = Session::get('currency_code', env('DEFAULT_CURRENCY', 'usd'));
        }

        // Try to get the requested currency
        $currency = \App\Models\Currency::where('code', $code)->first();

        // If currency not found, try to use default USD
        if (!$currency) {
            $currency = \App\Models\Currency::where('code', 'usd')->first();

            // If USD not found, create a default currency
            if (!$currency) {
                $currency = new \App\Models\Currency;
                $currency->name = 'US Dollar';
                $currency->code = 'usd';
                $currency->symbol = '$';
                $currency->rate = 1;
                $currency->alignment = 0;
                $currency->save();
            }
        }

        return $currency;
    }
}


if (!function_exists('productBasePriceWithoutTax')) {
    // min/base price of a product WITHOUT tax (for product cards)
    function productBasePriceWithoutTax($product, $formatted = false)
    {
        if (!$product || !isset($product->min_price)) {
            return $formatted ? formatPrice(0) : 0;
        }

        $price = $product->min_price;
        return $formatted ? formatPrice($price) : $price;
    }
}

if (!function_exists('productBasePrice')) {
    // min/base price of a product WITH tax (for checkout)
    function productBasePrice($product, $formatted = false)
    {
        if (!$product || !isset($product->min_price)) {
            return $formatted ? formatPrice(0) : 0;
        }

        $price = $product->min_price;
        $tax = 0;

        if ($product->taxes) {
            foreach ($product->taxes as $productTax) {
                if ($productTax->tax_type == 'percent') {
                    $tax += ($price * $productTax->tax_value) / 100;
                } elseif ($productTax->tax_type == 'flat') {
                    $tax += $productTax->tax_value;
                }
            }
        }

        $price += $tax;
        return $formatted ? formatPrice($price) : $price;
    }
}

if (!function_exists('discountedProductBasePriceWithoutTax')) {
    // min/base price of a product with discount WITHOUT tax (for product cards)
    function discountedProductBasePriceWithoutTax($product, $formatted = false)
    {
        if (!$product || !isset($product->min_price)) {
            return $formatted ? formatPrice(0) : 0;
        }

        $price = $product->min_price;

        $discount_applicable = false;

        if ($product->discount_start_date == null || $product->discount_end_date == null) {
            $discount_applicable = false;
        } elseif (
            strtotime(date('d-m-Y H:i:s')) >= $product->discount_start_date &&
            strtotime(date('d-m-Y H:i:s')) <= $product->discount_end_date
        ) {
            $discount_applicable = true;
        }

        if ($discount_applicable) {
            if ($product->discount_type == 'percent') {
                $price -= ($price * $product->discount_value) / 100;
            } elseif ($product->discount_type == 'flat') {
                $price -= $product->discount_value;
            }
        }

        return $formatted ? formatPrice($price) : $price;
    }
}

if (!function_exists('discountedProductBasePrice')) {
    // min/base price of a product with discount WITH tax (for checkout)
    function discountedProductBasePrice($product, $formatted = false)
    {
        if (!$product || !isset($product->min_price)) {
            return $formatted ? formatPrice(0) : 0;
        }

        $price = $product->min_price;

        $discount_applicable = false;

        if ($product->discount_start_date == null || $product->discount_end_date == null) {
            $discount_applicable = false;
        } elseif (
            strtotime(date('d-m-Y H:i:s')) >= $product->discount_start_date &&
            strtotime(date('d-m-Y H:i:s')) <= $product->discount_end_date
        ) {
            $discount_applicable = true;
        }

        if ($discount_applicable) {
            if ($product->discount_type == 'percent') {
                $price -= ($price * $product->discount_value) / 100;
            } elseif ($product->discount_type == 'flat') {
                $price -= $product->discount_value;
            }
        }

        if ($product->taxes) {
            foreach ($product->taxes as $product_tax) {
                if ($product_tax->tax_type == 'percent') {
                    $price += ($price * $product_tax->tax_value) / 100;
                } elseif ($product_tax->tax_type == 'flat') {
                    $price += $product_tax->tax_value;
                }
            }
        }

        return $formatted ? formatPrice($price) : $price;
    }
}

if (!function_exists('productMaxPriceWithoutTax')) {
    // max price of a product WITHOUT tax (for product cards)
    function productMaxPriceWithoutTax($product, $formatted = false)
    {
        if (!$product) {
            return $formatted ? formatPrice(0) : 0;
        }

        $price = $product->max_price ?? 0;
        return $formatted ? formatPrice($price) : $price;
    }
}

if (!function_exists('productMaxPrice')) {
    // max price of a product WITH tax (for checkout)
    function productMaxPrice($product, $formatted = false)
    {
        if (!$product) {
            return $formatted ? formatPrice(0) : 0;
        }

        $price = $product->max_price ?? 0;
        $tax = 0;

        if ($product->taxes) {
            foreach ($product->taxes as $productTax) {
                if ($productTax->tax_type == 'percent') {
                    $tax += ($price * $productTax->tax_value) / 100;
                } elseif ($productTax->tax_type == 'flat') {
                    $tax += $productTax->tax_value;
                }
            }
        }

        $price += $tax;
        return $formatted ? formatPrice($price) : $price;
    }
}

if (!function_exists('discountedProductMaxPriceWithoutTax')) {
    // max price of a product with discount WITHOUT tax (for product cards)
    function discountedProductMaxPriceWithoutTax($product, $formatted = false)
    {
        if (!$product) {
            return $formatted ? formatPrice(0) : 0;
        }

        $price = $product->max_price ?? 0;

        $discount_applicable = false;

        if ($product->discount_start_date == null || $product->discount_end_date == null) {
            $discount_applicable = false;
        } elseif (
            strtotime(date('d-m-Y H:i:s')) >= $product->discount_start_date &&
            strtotime(date('d-m-Y H:i:s')) <= $product->discount_end_date
        ) {
            $discount_applicable = true;
        }

        if ($discount_applicable) {
            if ($product->discount_type == 'percent') {
                $price -= ($price * $product->discount_value) / 100;
            } elseif ($product->discount_type == 'flat') {
                $price -= $product->discount_value;
            }
        }

        return $formatted ? formatPrice($price) : $price;
    }
}

if (!function_exists('discountedProductMaxPrice')) {
    // max price of a product with discount WITH tax (for checkout)
    function discountedProductMaxPrice($product, $formatted = false)
    {
        if (!$product) {
            return $formatted ? formatPrice(0) : 0;
        }

        $price = $product->max_price ?? 0;

        $discount_applicable = false;

        if ($product->discount_start_date == null || $product->discount_end_date == null) {
            $discount_applicable = false;
        } elseif (
            strtotime(date('d-m-Y H:i:s')) >= $product->discount_start_date &&
            strtotime(date('d-m-Y H:i:s')) <= $product->discount_end_date
        ) {
            $discount_applicable = true;
        }

        if ($discount_applicable) {
            if ($product->discount_type == 'percent') {
                $price -= ($price * $product->discount_value) / 100;
            } elseif ($product->discount_type == 'flat') {
                $price -= $product->discount_value;
            }
        }

        if ($product->taxes) {
            foreach ($product->taxes as $product_tax) {
                if ($product_tax->tax_type == 'percent') {
                    $price += ($price * $product_tax->tax_value) / 100;
                } elseif ($product_tax->tax_type == 'flat') {
                    $price += $product_tax->tax_value;
                }
            }
        }

        return $formatted ? formatPrice($price) : $price;
    }
}

if (!function_exists('discountPercentage')) {
    // return discount in %
    function discountPercentage($product)
    {
        if (!$product) {
            return 0;
        }

        $discountPercentage = $product->discount_value ?? 0;

        if ($product->discount_type != "percent") {
            $price = productBasePrice($product);
            $discountAmount = discountedProductBasePrice($product);
            $discountValue = $price - $discountAmount;
            $discountPercentage = ($discountValue * 100) / ($price > 0 ? $price : 1);
        }

        return round($discountPercentage);
    }
}

if (!function_exists('getTaxDisplayText')) {
    // Get tax display text with percentage for invoices
    function getTaxDisplayText($orderItems)
    {
        $taxPercentages = [];

        foreach ($orderItems as $item) {
            $product = $item->product_variation->productWithTrashed ?? $item->product_variation->product;

            foreach ($product->taxes as $productTax) {
                if ($productTax->tax_type == 'percent' && $productTax->tax_value > 0) {
                    $percentage = number_format($productTax->tax_value, 0);
                    if (!in_array($percentage, $taxPercentages)) {
                        $taxPercentages[] = $percentage;
                    }
                }
            }
        }

        if (empty($taxPercentages)) {
            return localize('Tax');
        }

        // Sort percentages for consistent display
        sort($taxPercentages);

        $taxText = implode(', ', array_map(function ($p) { return $p . '%'; }, $taxPercentages));
        return $taxText . ' VAT ';
    }
}

if (!function_exists('sellCountPercentage')) {
    // return sales count %
    function sellCountPercentage($product)
    {
        $sold = $product->total_sale_count;
        $target = (int) $product->sell_target;
        $salePercentage = ($sold * 100) / ($target > 0 ? $target : 1);
        return round($salePercentage);
    }
}

if (!function_exists('generateVariationOptions')) {
    //  generate combinations based on variations
    function generateVariationOptions($options, $withTrash = true)
    {

        if (count($options) == 0) {
            return $options;
        }
        $variation_ids = array();
        foreach ($options as $option) {

            $value_ids = array();
            if (isset($variation_ids[$option->variation_id])) {
                $value_ids = $variation_ids[$option->variation_id];
            }
            if (!in_array($option->variation_value_id, $value_ids)) {
                array_push($value_ids, $option->variation_value_id);
            }
            $variation_ids[$option->variation_id] = $value_ids;
        }
        $options = array();
        foreach ($variation_ids as $id => $values) {
            $variationValues = array();
            foreach ($values as $value) {
                $variationValue = VariationValue::find($value);
                $val = array(
                    'id'   => $value,
                    'name' => $variationValue->collectLocalization('name'),
                    'code' => $variationValue->color_code
                );
                array_push($variationValues, $val);
            }
            $data['id'] = $id;

            if($withTrash == true) {
                $data['name'] = Variation::withTrashed()->find($id)->collectLocalization('name');
            }else{
                $data['name'] = Variation::find($id) ? Variation::find($id)->collectLocalization('name') : null;
            }
            if( $data['name']){
                $data['values'] = $variationValues;
            }else{
                $data['values'] = $variationValues;
            }

            array_push($options, $data);
        }
        return $options;
    }
}

if (!function_exists('variationPrice')) {
    // return price of a variation
    function variationPrice($product, $variation, $addTax = true)
    {
        $price = $variation->price;

        if ($addTax) {
            foreach ($product->taxes as $product_tax) {
                if ($product_tax->tax_type == 'percent') {
                    $price += ($price * $product_tax->tax_value) / 100;
                } elseif ($product_tax->tax_type == 'flat') {
                    $price += $product_tax->tax_value;
                }
            }
        }
        return $price;
    }
}

if (!function_exists('variationDiscountedPrice')) {
    // return discounted price of a variation
    function variationDiscountedPrice($product, $variation, $addTax = true)
    {
        $price = $variation->price;

        $discount_applicable = false;


        if ($product->discount_start_date == null || $product->discount_end_date == null) {
            $discount_applicable = false;
        } elseif (
            strtotime(date('d-m-Y H:i:s')) >= $product->discount_start_date &&
            strtotime(date('d-m-Y H:i:s')) <= $product->discount_end_date
        ) {
            $discount_applicable = true;
        }

        if ($discount_applicable) {
            if ($product->discount_type == 'percent') {
                $price -= ($price * $product->discount_value) / 100;
            } elseif ($product->discount_type == 'flat') {
                $price -= $product->discount_value;
            }
        }

        if ($addTax) {
            foreach ($product->taxes as $product_tax) {
                if ($product_tax->tax_type == 'percent') {
                    $price += ($price * $product_tax->tax_value) / 100;
                } elseif ($product_tax->tax_type == 'flat') {
                    $price += $product_tax->tax_value;
                }
            }
        }

        return $price;
    }
}


// file upload
if (!function_exists('fileUpload')) {
    function fileUpload($path, $file, $change_name = false)
    {

        $fileName = '';
        if (!$file) {
            return $fileName;
        }

        $original_name = $file->getClientOriginalName();
        if ($change_name) {
            $name = $original_name;
        } else {
            $str = str_replace(' ', '-', $original_name);
            $name = time() . '_' . $str;
        }

        if (!file_exists($path)) {
            mkdir($path, 0777, true);
        }

        $file->move($path, $name);
        $fileName = $path . $name;

        return $fileName;
    }
}

// file update
if (!function_exists('fileUpdate')) {
    function fileUpdate($databaseFile, $path, $file)
    {

        $fileName = "";


        if ($file) {
            $fileName = fileUpload($path, $file);

            if ($databaseFile && file_exists($databaseFile)) {

                unlink($databaseFile);
            }
        } elseif (!$file and $databaseFile) {
            $fileName = $databaseFile;
        }


        return $fileName;
    }
}

if (!function_exists('variationTaxAmount')) {
    // return tax of a variation
    function variationTaxAmount($product, $variation)
    {
        $price = $variation->price;
        $tax   = 0;

        $discount_applicable = false;

        if ($product->discount_start_date == null || $product->discount_end_date == null) {
            $discount_applicable = false;
        } elseif (
            strtotime(date('d-m-Y H:i:s')) >= $product->discount_start_date &&
            strtotime(date('d-m-Y H:i:s')) <= $product->discount_end_date
        ) {
            $discount_applicable = true;
        }

        if ($discount_applicable) {
            if ($product->discount_type == 'percent') {
                $price -= ($price * $product->discount_value) / 100;
            } elseif ($product->discount_type == 'flat') {
                $price -= $product->discount_value;
            }
        }

        foreach ($product->taxes as $product_tax) {
            if ($product_tax->tax_type == 'percent') {
                $tax += ($price * $product_tax->tax_value) / 100;
            } elseif ($product_tax->tax_type == 'flat') {
                $tax += $product_tax->tax_value;
            }
        }

        return $tax;
    }
}

if (!function_exists('getSubTotal')) {
    // return sub total price
    function getSubTotal($carts, $couponDiscount = true, $couponCode = '', $addTax = true)
    {
        $price = 0;
        $amount = 0;
        if (count($carts) > 0) {
            foreach ($carts as $cart) {
                $product    = $cart->product_variation->product;
                $variation  = $cart->product_variation;

                $discountedVariationPriceWithTax = variationDiscountedPrice($product, $variation, $addTax);
                $price += (float) $discountedVariationPriceWithTax * $cart->qty;
            }

            # calculate coupon discount
            if ($couponDiscount) {
                $amount = getCouponDiscount($price, $couponCode);
            }
        }

        return $price - $amount;
    }
}

if (!function_exists('setCoupon')) {
    // set coupon code in session and cookie
    function setCoupon($coupon)
    {
        // Set in session for immediate availability
        session(['coupon_code' => $coupon->code]);

        // Also set in cookie for persistence using Laravel's cookie helper
        try {
            $cookie = cookie('coupon_code', $coupon->code, 60 * 24 * 7); // 7 days
            \Cookie::queue($cookie);
        } catch (\Exception $e) {
            // Fallback to PHP setcookie
            $theTime = time() + 86400 * 7;
            setcookie('coupon_code', $coupon->code, $theTime, '/');
        }
    }
}

if (!function_exists('removeCoupon')) {
    // remove coupon code from session and cookie
    function removeCoupon()
    {
        // Remove from session
        session()->forget('coupon_code');

        // Remove Laravel cookie
        \Cookie::queue(\Cookie::forget('coupon_code'));

        // Remove PHP cookie
        if (isset($_COOKIE["coupon_code"])) {
            setcookie("coupon_code", "", time() - 3600, '/');
            unset($_COOKIE["coupon_code"]);
        }
    }
}

if (!function_exists('getCoupon')) {
    // get coupon code from session, cookie, or header
    function getCoupon()
    {
        // Check header first (for API requests)
        if (request()->hasHeader("Coupon-Code")) {
            return request()->header("Coupon-Code");
        }

        // Check session first (immediate availability)
        if (session('coupon_code')) {
            return session('coupon_code');
        }

        // Fallback to Laravel cookie
        if (request()->cookie('coupon_code')) {
            return request()->cookie('coupon_code');
        }

        // Fallback to PHP cookie
        if (isset($_COOKIE["coupon_code"])) {
            return $_COOKIE["coupon_code"];
        }

        return '';
    }
}

if (!function_exists('getCouponDiscount')) {
    // return Coupon Discount amount (LEGACY - for backward compatibility)
    function getCouponDiscount($subTotal, $code = '')
    {
        $amount = 0;
        $coupon = Coupon::where('code', $code)->first();
        if ($coupon) {
            $date = strtotime(date('d-m-Y H:i:s'));
            # check if coupon is not expired
            if ($coupon->start_date <= $date && $coupon->end_date >= $date) {
                if ($coupon->discount_type == 'flat') {
                    $amount = (float) $coupon->discount_value;
                } else {
                    $amount = ((float) $coupon->discount_value * $subTotal) / 100;
                    if ($amount > (float) $coupon->max_discount_amount) {
                        $amount = (float) $coupon->max_discount_amount;
                    }
                }
            } else {
                removeCoupon();
            }
        } else {
            removeCoupon();
        }

        return $amount;
    }
}

if (!function_exists('getCouponDiscountWithRestrictions')) {
    // return Coupon Discount amount with proper product/category filtering
    function getCouponDiscountWithRestrictions($carts, $code = '')
    {
        $amount = 0;
        if (empty($code) || !$carts || count($carts) == 0) {
            return $amount;
        }

        $coupon = \App\Models\Coupon::where('code', $code)->first();
        if (!$coupon) {
            return $amount;
        }

        $date = strtotime(date('d-m-Y H:i:s'));
        # check if coupon is not expired
        if ($coupon->start_date > $date || $coupon->end_date < $date) {
            return $amount;
        }

        // Check if coupon has product/category restrictions
        $hasRestrictions = !empty($coupon->product_ids) || !empty($coupon->category_ids);

        if ($hasRestrictions) {
            // RESTRICTED COUPON: Calculate discount only on eligible products
            $eligibleSubtotal = 0;

            foreach ($carts as $cart) {
                if (isProductEligibleForCoupon($cart, $coupon)) {
                    $product = $cart->product_variation->product;
                    $unitPrice = $product->min_price ?? 0;
                    $eligibleSubtotal += $unitPrice * $cart->qty;
                }
            }

            // Calculate discount on eligible subtotal only
            if ($coupon->discount_type == 'flat') {
                $amount = (float) $coupon->discount_value;
            } else {
                $amount = ((float) $coupon->discount_value * $eligibleSubtotal) / 100;
                if ($amount > (float) $coupon->max_discount_amount && $coupon->max_discount_amount > 0) {
                    $amount = (float) $coupon->max_discount_amount;
                }
            }
        } else {
            // UNRESTRICTED COUPON: Calculate discount on entire subtotal
            $totalSubtotal = 0;
            foreach ($carts as $cart) {
                $product = $cart->product_variation->product;
                $unitPrice = $product->min_price ?? 0;
                $totalSubtotal += $unitPrice * $cart->qty;
            }

            if ($coupon->discount_type == 'flat') {
                $amount = (float) $coupon->discount_value;
            } else {
                $amount = ((float) $coupon->discount_value * $totalSubtotal) / 100;
                if ($amount > (float) $coupon->max_discount_amount && $coupon->max_discount_amount > 0) {
                    $amount = (float) $coupon->max_discount_amount;
                }
            }
        }

        return $amount;
    }
}

if (!function_exists('getCouponDiscountFromOrderItems')) {
    // Calculate coupon discount from order items with proper product/category filtering
    function getCouponDiscountFromOrderItems($orderItems, $code = '')
    {
        $amount = 0;
        if (empty($code) || !$orderItems || count($orderItems) == 0) {
            return $amount;
        }

        $coupon = \App\Models\Coupon::where('code', $code)->first();
        if (!$coupon) {
            return $amount;
        }

        // Check if coupon has product/category restrictions
        $hasRestrictions = !empty($coupon->product_ids) || !empty($coupon->category_ids);

        if ($hasRestrictions) {
            // RESTRICTED COUPON: Calculate discount only on eligible products
            $eligibleSubtotal = 0;

            foreach ($orderItems as $orderItem) {
                if (isOrderItemEligibleForCoupon($orderItem, $coupon)) {
                    $product = $orderItem->product_variation->productWithTrashed ?? $orderItem->product_variation->product;
                    $unitPrice = $product->base_price ?? $product->min_price ?? 0;
                    $eligibleSubtotal += $unitPrice * $orderItem->qty;
                }
            }

            // Calculate discount on eligible subtotal only
            if ($coupon->discount_type == 'flat') {
                $amount = (float) $coupon->discount_value;
            } else {
                $amount = ((float) $coupon->discount_value * $eligibleSubtotal) / 100;
                if ($amount > (float) $coupon->max_discount_amount && $coupon->max_discount_amount > 0) {
                    $amount = (float) $coupon->max_discount_amount;
                }
            }
        } else {
            // UNRESTRICTED COUPON: Calculate discount on entire subtotal
            $totalSubtotal = 0;
            foreach ($orderItems as $orderItem) {
                $product = $orderItem->product_variation->productWithTrashed ?? $orderItem->product_variation->product;
                $unitPrice = $product->base_price ?? $product->min_price ?? 0;
                $totalSubtotal += $unitPrice * $orderItem->qty;
            }

            if ($coupon->discount_type == 'flat') {
                $amount = (float) $coupon->discount_value;
            } else {
                $amount = ((float) $coupon->discount_value * $totalSubtotal) / 100;
                if ($amount > (float) $coupon->max_discount_amount && $coupon->max_discount_amount > 0) {
                    $amount = (float) $coupon->max_discount_amount;
                }
            }
        }

        return $amount;
    }
}

if (!function_exists('isOrderItemEligibleForCoupon')) {
    // Check if an order item is eligible for a specific coupon
    function isOrderItemEligibleForCoupon($orderItem, $coupon)
    {
        if (!$coupon) {
            return false;
        }

        // If coupon has no restrictions, it applies to all products
        if (empty($coupon->product_ids) && empty($coupon->category_ids)) {
            return true;
        }

        $product = $orderItem->product_variation->productWithTrashed ?? $orderItem->product_variation->product;

        // Check product-specific eligibility
        if ($coupon->product_ids) {
            $product_ids = json_decode($coupon->product_ids);
            if (in_array($product->id, $product_ids)) {
                return true;
            }
        }

        // Check category-specific eligibility
        if ($coupon->category_ids) {
            $category_ids = json_decode($coupon->category_ids);

            // Get all subcategory IDs for each main category in coupon
            $allCategoryIds = collect($category_ids)->map(function($catId) {
                $cat = \App\Models\Category::find($catId);
                if ($cat) {
                    // Get all descendant category IDs (including itself)
                    $ids = [$cat->id];
                    $ids = array_merge($ids, getAllSubcategoryIds($cat));
                    return $ids;
                }
                return [$catId];
            })->flatten()->unique()->toArray();

            $product_categories = $product->product_categories ?? $product->categories ?? collect();
            foreach ($product_categories as $product_category) {
                $categoryId = $product_category->category_id ?? $product_category->id;
                if (in_array($categoryId, $allCategoryIds)) {
                    return true;
                }
            }
        }

        return false;
    }
}

if (!function_exists('validateCouponForProductsAndCategories')) {
    # check coupon for products & categories
    function validateCouponForProductsAndCategories($cartItems, $coupon)
    {
        if ($coupon->product_ids) {
            $product_ids = json_decode($coupon->product_ids);
            foreach ($cartItems as $key => $cartItem) {
                if (in_array($cartItem->product_variation->product_id, $product_ids)) {
                    return true;
                }
            }
        }

        if ($coupon->category_ids) {
            $category_ids = json_decode($coupon->category_ids);
            // Get all subcategory IDs for each main category in coupon
            $allCategoryIds = collect($category_ids)->map(function($catId) {
                $cat = \App\Models\Category::find($catId);
                if ($cat) {
                    // Get all descendant category IDs (including itself)
                    $ids = [$cat->id];
                    $ids = array_merge($ids, getAllSubcategoryIds($cat));
                    return $ids;
                }
                return [$catId];
            })->flatten()->unique()->toArray();

            foreach ($cartItems as $key => $cartItem) {
                $product_categories = $cartItem->product_variation->product->product_categories;
                foreach ($product_categories as $key => $product_category) {
                    if (in_array($product_category->category_id, $allCategoryIds)) {
                        return true;
                    }
                }
            }
        }

        return false;
    }
}

if (!function_exists('getAllSubcategoryIds')) {
    // Recursively get all subcategory IDs for a given category
    function getAllSubcategoryIds($category)
    {
        $ids = [];
        foreach ($category->childrenCategories as $child) {
            $ids[] = $child->id;
            $ids = array_merge($ids, getAllSubcategoryIds($child));
        }
        return $ids;
    }
}

if (!function_exists('checkCouponValidityForCheckout')) {
    // check coupon validity For Checkout
    function checkCouponValidityForCheckout($carts)
    {
        if (getCoupon() != '') {
            $date = strtotime(date('d-m-Y H:i:s'));
            $coupon = Coupon::where('code', getCoupon())->first();
            if ($coupon) {
                # total coupon usage
                $totalCouponUsage = CouponUsage::where('coupon_code', $coupon->code)->sum('usage_count');
                if ($totalCouponUsage == $coupon->total_usage_limit) {
                    # coupon usage limit reached
                    removeCoupon();
                    return [
                        'status'    => false,
                        'message'   => localize('Total usage limit has been reached for the coupon')
                    ];
                }

                # coupon usage by user (only check for authenticated users)
                if (auth()->check()) {
                    $couponUsageByUser = CouponUsage::where('user_id', auth()->user()->id)->where('coupon_code', $coupon->code)->first();
                    if (!is_null($couponUsageByUser)) {
                        if ($couponUsageByUser->usage_count ==  $coupon->customer_usage_limit) {
                            removeCoupon();
                            return [
                                'status'    => false,
                                'message'   => localize('You have used this coupon for maximum time')
                            ];
                        }
                    }
                }

                # check if coupon is expired
                if ($coupon->start_date <= $date && $coupon->end_date >= $date) {
                    $subTotal = (float) getSubTotal($carts, false);
                    if ($subTotal >= (float) $coupon->min_spend) {
                        # check if coupon is for categories or products
                        if ($coupon->product_ids || $coupon->category_ids) {
                            if (!validateCouponForProductsAndCategories($carts, $coupon)) {
                                # coupon not valid for your cart items
                                removeCoupon();
                                return [
                                    'status'    => false,
                                    'message'   => localize('Coupon is not valid for the products')
                                ];
                            }

                            return [
                                'status'    => true,
                                'message'   => ''
                            ];
                        }

                        return [
                            'status'    => true,
                            'message'   => ''
                        ];
                    } else {
                        # min amount not reached
                        removeCoupon();
                        return [
                            'status'    => false,
                            'message'   => localize('Minimum order amount is not reached to use this coupon')
                        ];
                    }
                } else {
                    # expired
                    removeCoupon();
                    return [
                        'status'    => false,
                        'message'   => localize('Coupon has been expired')
                    ];
                }
            } else {
                # coupon not found
                removeCoupon();
                return [
                    'status'    => false,
                    'message'   => localize('Coupon is not valid')
                ];
            }
        }

        // coupon not set - so return true
        return [
            'status'    => true,
            'message'   => ''
        ];
    }
}

if (!function_exists('getTotalTax')) {
    // get Total Tax from carts (DEPRECATED - use getTotalTaxWithCouponDiscount for accurate calculation)
    function getTotalTax($carts)
    {
        $tax = 0;
        if ($carts) {

            foreach ($carts as $cart) {
                $product    = $cart->product_variation->product;
                $variation  = $cart->product_variation;

                $variationTaxAmount = variationTaxAmount($product, $variation);
                $tax += (float) $variationTaxAmount * $cart->qty;
            }
        }
        return $tax;
    }
}

if (!function_exists('getTotalTaxWithCouponDiscount')) {
    // get Total Tax from carts with proper coupon discount handling and product/category filtering
    function getTotalTaxWithCouponDiscount($carts, $couponCode = '')
    {
        $tax = 0;
        if (!$carts || count($carts) == 0) {
            return $tax;
        }

        // Get coupon details if provided
        $coupon = null;
        $couponDiscount = 0;
        if (!empty($couponCode)) {
            $coupon = \App\Models\Coupon::where('code', $couponCode)->first();
            if ($coupon) {
                // Check if coupon has product/category restrictions
                $hasRestrictions = !empty($coupon->product_ids) || !empty($coupon->category_ids);

                if ($hasRestrictions) {
                    // RESTRICTED COUPON: Apply discount only to eligible products
                    $eligibleSubtotal = 0;

                    // Calculate subtotal for eligible products only
                    foreach ($carts as $cart) {
                        if (isProductEligibleForCoupon($cart, $coupon)) {
                            $product = $cart->product_variation->product;
                            $unitPriceWithoutTax = $product->min_price ?? 0;
                            $eligibleSubtotal += $unitPriceWithoutTax * $cart->qty;
                        }
                    }

                    // Calculate coupon discount on eligible products only
                    $couponDiscount = getCouponDiscount($eligibleSubtotal, $couponCode);

                    // Apply tax calculation
                    foreach ($carts as $cart) {
                        $product = $cart->product_variation->product;

                        // Check if product has taxes
                        if (!$product->taxes || $product->taxes->count() == 0) {
                            continue; // Skip non-taxable products
                        }

                        $unitPriceWithoutTax = $product->min_price ?? 0;
                        $itemSubtotal = $unitPriceWithoutTax * $cart->qty;
                        $itemDiscountedPrice = $itemSubtotal; // Default: no discount

                        // Apply coupon discount only to eligible products
                        if (isProductEligibleForCoupon($cart, $coupon)) {
                            $itemDiscountRatio = $eligibleSubtotal > 0 ? ($itemSubtotal / $eligibleSubtotal) : 0;
                            $itemCouponDiscount = $couponDiscount * $itemDiscountRatio;
                            $itemDiscountedPrice = $itemSubtotal - $itemCouponDiscount;
                        }

                        // Calculate tax on discounted price
                        foreach ($product->taxes as $productTax) {
                            if ($productTax->tax_type == 'percent' && $productTax->tax_value > 0) {
                                $tax += ($itemDiscountedPrice * $productTax->tax_value) / 100;
                            } elseif ($productTax->tax_type == 'flat' && $productTax->tax_value > 0) {
                                $tax += $productTax->tax_value * $cart->qty;
                            }
                        }
                    }
                } else {
                    // UNRESTRICTED COUPON: Apply to all products (original logic)
                    $subtotalWithoutTax = 0;
                    foreach ($carts as $cart) {
                        $product = $cart->product_variation->product;
                        $unitPriceWithoutTax = $product->min_price ?? 0;
                        $subtotalWithoutTax += $unitPriceWithoutTax * $cart->qty;
                    }

                    $couponDiscount = getCouponDiscount($subtotalWithoutTax, $couponCode);

                    foreach ($carts as $cart) {
                        $product = $cart->product_variation->product;

                        if (!$product->taxes || $product->taxes->count() == 0) {
                            continue;
                        }

                        $unitPriceWithoutTax = $product->min_price ?? 0;
                        $itemSubtotal = $unitPriceWithoutTax * $cart->qty;

                        $itemDiscountRatio = $subtotalWithoutTax > 0 ? ($itemSubtotal / $subtotalWithoutTax) : 0;
                        $itemCouponDiscount = $couponDiscount * $itemDiscountRatio;
                        $itemDiscountedPrice = $itemSubtotal - $itemCouponDiscount;

                        foreach ($product->taxes as $productTax) {
                            if ($productTax->tax_type == 'percent' && $productTax->tax_value > 0) {
                                $tax += ($itemDiscountedPrice * $productTax->tax_value) / 100;
                            } elseif ($productTax->tax_type == 'flat' && $productTax->tax_value > 0) {
                                $tax += $productTax->tax_value * $cart->qty;
                            }
                        }
                    }
                }
            }
        } else {
            // NO COUPON: Calculate tax on original prices
            foreach ($carts as $cart) {
                $product = $cart->product_variation->product;

                if (!$product->taxes || $product->taxes->count() == 0) {
                    continue;
                }

                $unitPriceWithoutTax = $product->min_price ?? 0;
                $itemSubtotal = $unitPriceWithoutTax * $cart->qty;

                foreach ($product->taxes as $productTax) {
                    if ($productTax->tax_type == 'percent' && $productTax->tax_value > 0) {
                        $tax += ($itemSubtotal * $productTax->tax_value) / 100;
                    } elseif ($productTax->tax_type == 'flat' && $productTax->tax_value > 0) {
                        $tax += $productTax->tax_value * $cart->qty;
                    }
                }
            }
        }

        return $tax;
    }
}

if (!function_exists('isProductEligibleForCoupon')) {
    // Check if a single cart item is eligible for a specific coupon
    function isProductEligibleForCoupon($cartItem, $coupon)
    {
        if (!$coupon) {
            return false;
        }

        // If coupon has no restrictions, it applies to all products
        if (empty($coupon->product_ids) && empty($coupon->category_ids)) {
            return true;
        }

        $product = $cartItem->product_variation->product;

        // Check product-specific eligibility
        if ($coupon->product_ids) {
            $product_ids = json_decode($coupon->product_ids);
            if (in_array($product->id, $product_ids)) {
                return true;
            }
        }

        // Check category-specific eligibility
        if ($coupon->category_ids) {
            $category_ids = json_decode($coupon->category_ids);

            // Get all subcategory IDs for each main category in coupon
            $allCategoryIds = collect($category_ids)->map(function($catId) {
                $cat = \App\Models\Category::find($catId);
                if ($cat) {
                    // Get all descendant category IDs (including itself)
                    $ids = [$cat->id];
                    $ids = array_merge($ids, getAllSubcategoryIds($cat));
                    return $ids;
                }
                return [$catId];
            })->flatten()->unique()->toArray();

            $product_categories = $product->product_categories;
            foreach ($product_categories as $product_category) {
                if (in_array($product_category->category_id, $allCategoryIds)) {
                    return true;
                }
            }
        }

        return false;
    }
}

if (!function_exists('getTaxableSubtotal')) {
    // get subtotal for taxable items only (without tax)
    function getTaxableSubtotal($carts)
    {
        $subtotal = 0;
        if (!$carts || count($carts) == 0) {
            return $subtotal;
        }

        foreach ($carts as $cart) {
            $product = $cart->product_variation->product;

            // Check if product has taxes
            if ($product->taxes && $product->taxes->count() > 0) {
                $hasTax = false;
                foreach ($product->taxes as $tax) {
                    if (($tax->tax_type == 'percent' && $tax->tax_value > 0) ||
                        ($tax->tax_type == 'flat' && $tax->tax_value > 0)) {
                        $hasTax = true;
                        break;
                    }
                }

                if ($hasTax) {
                    // Get base price
                    $basePrice = $product->base_price ?? $product->min_price ?? 0;

                    // Apply discount if active
                    $discountedPrice = $basePrice;
                    if ($product->discount_start_date && $product->discount_end_date && $product->discount_value > 0) {
                        if (strtotime(date('d-m-Y H:i:s')) >= $product->discount_start_date &&
                            strtotime(date('d-m-Y H:i:s')) <= $product->discount_end_date) {
                            if ($product->discount_type == 'percent') {
                                $discountedPrice = $basePrice - (($basePrice * $product->discount_value) / 100);
                            } elseif ($product->discount_type == 'flat') {
                                $discountedPrice = $basePrice - $product->discount_value;
                            }
                        }
                    }

                    $subtotal += $discountedPrice * $cart->qty;
                }
            }
        }

        return $subtotal;
    }
}

if (!function_exists('getNonTaxableSubtotal')) {
    // get subtotal for non-taxable items only (without tax)
    function getNonTaxableSubtotal($carts)
    {
        $subtotal = 0;
        if (!$carts || count($carts) == 0) {
            return $subtotal;
        }

        foreach ($carts as $cart) {
            $product = $cart->product_variation->product;

            // Check if product has NO taxes or taxes with 0 value
            $hasTax = false;
            if ($product->taxes && $product->taxes->count() > 0) {
                foreach ($product->taxes as $tax) {
                    if (($tax->tax_type == 'percent' && $tax->tax_value > 0) ||
                        ($tax->tax_type == 'flat' && $tax->tax_value > 0)) {
                        $hasTax = true;
                        break;
                    }
                }
            }

            if (!$hasTax) {
                // Get base price
                $basePrice = $product->base_price ?? $product->min_price ?? 0;

                // Apply discount if active
                $discountedPrice = $basePrice;
                if ($product->discount_start_date && $product->discount_end_date && $product->discount_value > 0) {
                    if (strtotime(date('d-m-Y H:i:s')) >= $product->discount_start_date &&
                        strtotime(date('d-m-Y H:i:s')) <= $product->discount_end_date) {
                        if ($product->discount_type == 'percent') {
                            $discountedPrice = $basePrice - (($basePrice * $product->discount_value) / 100);
                        } elseif ($product->discount_type == 'flat') {
                            $discountedPrice = $basePrice - $product->discount_value;
                        }
                    }
                }

                $subtotal += $discountedPrice * $cart->qty;
            }
        }

        return $subtotal;
    }
}

if (!function_exists('getScheduledDeliveryType')) {
    // delivery type Status
    function getScheduledDeliveryType()
    {
        return "scheduled";
    }
}

if (!function_exists('paidPaymentStatus')) {
    // paid Payment Status
    function paidPaymentStatus()
    {
        return "paid";
    }
}
if (!function_exists('unpaidPaymentStatus')) {
    // unpaid Payment Status
    function unpaidPaymentStatus()
    {
        return "unpaid";
    }
}

if (!function_exists('orderPlacedStatus')) {
    // orderPlacedStatus
    function orderPlacedStatus()
    {
        return "order_placed";
    }
}
if (!function_exists('orderPendingStatus')) {
    // orderPendingStatus
    function orderPendingStatus()
    {
        return "pending";
    }
}
if (!function_exists('orderProcessingStatus')) {
    // orderProcessingStatus
    function orderProcessingStatus()
    {
        return "processing";
    }
}

// orderIsPickedUpStatus Order status
if (!function_exists('orderPickedUpStatus')) {
    // orderIsPickedUpStatus
    function orderPickedUpStatus()
    {
        return "picked_up";
    }
}

//  OutForDelivery Order status
if (!function_exists('orderOutForDeliveryStatus')) {
    // orderProcessingStatus
    function orderOutForDeliveryStatus()
    {
        return "out_for_delivery";
    }
}

if (!function_exists('orderDeliveredStatus')) {
    // order Delivered Status
    function orderDeliveredStatus()
    {
        return "delivered";
    }
}

if (!function_exists('orderCancelledStatus')) {
    // order cancelled Status
    function orderCancelledStatus()
    {
        return "cancelled";
    }
}

if (!function_exists('recaptchaValidation')) {
    // recaptchaValidation
    function recaptchaValidation($request)
    {
        $score = 1;
        if (getSetting('enable_recaptcha') == 1){
            $score = RecaptchaV3::verify($request->get('g-recaptcha-response'), 'recaptcha_token');
        }
        return $score;
    }
}


if (!function_exists('allMonths')) {
    function allMonths(){
        $month = [];

        for ($m=1; $m<=12; $m++) {
             $month[] = date('F', mktime(0,0,0,$m, 1, date('Y')));
        }

        return $month;
    }
}

if (!function_exists('getPaymentAmount')) {
    function getPaymentAmount(){
        // Try to get amount from session first (new flow), then from database (old flow)
        $amount = session('payment_amount');
        if (!$amount && session('payment_type') == 'order_payment') {
            $orderGroup = \App\Models\OrderGroup::where('order_code', session('order_code'))->first(['grand_total_amount']);
            $amount = $orderGroup ? $orderGroup->grand_total_amount : 0;
        }
        return $amount ?: 0;
    }
}

 # module check
 if(!function_exists('isModuleActive')){
    function isModuleActive($name) {

        $module = Module::find($name);
        if($module) {
            $status = $module->isEnabled();
            if($status == true) {
                $modulePath = $module->getPath() .'/Providers/RouteServiceProvider.php';
                if(file_exists($modulePath)) {
                    $module = EnmartModule::where('name', $name)->first();
                    if($module) {
                        if($module->is_default == 1) {
                            $status = true;
                        }
                        if($module->is_paid == 1) {
                            $status = $module->purchase_code && $module->domain  ? true : false;
                        }
                    }else{
                        $status = false;
                    }
                }else{
                    $status = false;
                }
            }
        }
        return $status;
    }
 }

 # text to a slug.
if(!function_exists('convertToSlug')) {
    function convertToSlug($text) {

        $text = mb_strtolower(trim(preg_replace('~[^\pL\d]+~u', ' ', $text)));

        // Remove diacritics from the text
        $textV = removeDiacritics($text);

        // Replace spaces and special characters with dashes
        $slug = preg_replace('/[^a-z0-9-]+/', '-', strtolower($textV));

        // Remove leading and trailing dashes
        $slug = trim($slug, '-');
        if($slug == '') {
            $normalize = new SlugNormalizer;
            $slug = $normalize->normalize($text);
        }
        return $slug;
    }
}

# This function removes diacritics from Vietnamese text.
if(!function_exists('removeDiacritics')) {
    function removeDiacritics($text) {
        $diacritics = array(
            'à' => 'a', 'á' => 'a', 'ạ' => 'a', 'ả' => 'a', 'ã' => 'a', 'â' => 'a', 'ầ' => 'a', 'ấ' => 'a', 'ậ' => 'a', 'ẩ' => 'a', 'ẫ' => 'a', 'ă' => 'a', 'ằ' => 'a', 'ắ' => 'a', 'ặ' => 'a', 'ẳ' => 'a', 'ẵ' => 'a',
            'è' => 'e', 'é' => 'e', 'ẹ' => 'e', 'ẻ' => 'e', 'ẽ' => 'e', 'ê' => 'e', 'ề' => 'e', 'ế' => 'e', 'ệ' => 'e', 'ể' => 'e', 'ễ' => 'e',
            'ì' => 'i', 'í' => 'i', 'ị' => 'i', 'ỉ' => 'i', 'ĩ' => 'i',
            'ò' => 'o', 'ó' => 'o', 'ọ' => 'o', 'ỏ' => 'o', 'õ' => 'o', 'ô' => 'o', 'ồ' => 'o', 'ố' => 'o', 'ộ' => 'o', 'ổ' => 'o', 'ỗ' => 'o', 'ơ' => 'o', 'ờ' => 'o', 'ớ' => 'o', 'ợ' => 'o', 'ở' => 'o', 'ỡ' => 'o',
            'ù' => 'u', 'ú' => 'u', 'ụ' => 'u', 'ủ' => 'u', 'ũ' => 'u', 'ư' => 'u', 'ừ' => 'u', 'ứ' => 'u', 'ự' => 'u', 'ử' => 'u', 'ữ' => 'u',
            'ỳ' => 'y', 'ý' => 'y', 'ỵ' => 'y', 'ỷ' => 'y', 'ỹ' => 'y',
            'đ' => 'd',
        );
        return strtr($text, $diacritics);
    }
}

if(!function_exists('active_themes_array')){
    function active_themes_array(){
       return json_decode(getSetting('active_themes')) ?? [1];
    }
}

if(!function_exists('current_theme')){
    function current_theme($code = null){
        if($code){
            return $theme = Theme::where('code', $code)->first();
        }
        $code = session()->get('theme') ?? 'default';
        $theme = Theme::where('code', $code)->first();

        return $theme;

    }
}


if(!function_exists('appStatic')){
    function appStatic(){

        return new \App\Utils\AppStatic();

    }
}

if (!function_exists('formatOrderDate')) {
    /**
     * Format order date in application timezone
     *
     * @param string|Carbon $date
     * @param string $format
     * @return string
     */
    function formatOrderDate($date, $format = 'd M, Y')
    {
        if (!$date) {
            return '-';
        }

        try {
            // Convert to Carbon instance if it's a string
            if (is_string($date)) {
                $carbonDate = \Carbon\Carbon::parse($date);
            } else {
                $carbonDate = $date;
            }

            // Ensure it's in the application timezone
            $carbonDate = $carbonDate->setTimezone(config('app.timezone'));

            return $carbonDate->format($format);
        } catch (\Exception $e) {
            return '-';
        }
    }
}

if (!function_exists('formatOrderDateTime')) {
    /**
     * Format order date and time in application timezone
     *
     * @param string|Carbon $date
     * @param string $format
     * @return string
     */
    function formatOrderDateTime($date, $format = 'd M, Y h:i A')
    {
        return formatOrderDate($date, $format);
    }
}

if (!function_exists('getAppTimezone')) {
    /**
     * Get application timezone
     *
     * @return string
     */
    function getAppTimezone()
    {
        return config('app.timezone', 'UTC');
    }
}

if (!function_exists('getCurrentTimeInAppTimezone')) {
    /**
     * Get current time in application timezone
     *
     * @param string $format
     * @return string
     */
    function getCurrentTimeInAppTimezone($format = 'd M, Y h:i A')
    {
        return now()->setTimezone(config('app.timezone'))->format($format);
    }
}


if(!function_exists('getCategoriesByIds'))
{

    function getCategoriesByIds($category_ids)
    {
        return Category::whereIn('id', $category_ids)->get();
    }
}


if(!function_exists('getProductsByIds'))
{
    function getProductsByIds($product_ids)
    {
        return Product::whereIn('id', $product_ids)->get();
    }
}


if(!function_exists('getThemeId'))
{
    function getThemeId($code)
    {
        return Theme::where('code', $code)->first()?->id;
    }
}




if(!function_exists('CountProductsByCategoryAndThemeId'))
{
    function CountProductsByCategoryAndThemeId($themeId, $categoryId )
    {
        // ProductTheme.php
        $query = ProductTheme::selectRaw("count(products.id) as total_product")
            ->where('product_themes.theme_id', $themeId)
            ->join("products", "products.id", "=", "product_themes.product_id")
            ->join("product_categories", "product_categories.product_id", "=", "products.id")
            ->join("categories", "categories.id", "=", "product_categories.category_id")
            ->where("categories.id", $categoryId)
            ->groupBy("product_themes.theme_id")
            ->value('total_product') ?? 0;


        return $query;
    }
}


