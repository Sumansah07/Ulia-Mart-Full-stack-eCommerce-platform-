...
@section('scripts')
    <script>
        ...
        $(document).ready(function() {
            console.log('POS Page: Document ready');

            // Force hide preloader for POS page
            setTimeout(function() {
                console.log('POS Page: Hiding preloader');
                $('#preloader').fadeOut('fast');
            }, 500);

            console.log('POS Page: Starting to get products');
            getPosProducts();
        });

        // get pos products
        function getPosProducts() {
            console.log('getPosProducts: Starting function');
            let url = '{{ route('admin.pos.products') }}';
            console.log('getPosProducts: URL is', url);

            $('.pos-products').empty();
            $('.pos-load-more').addClass('d-none')

            // Show loading indicator
            $('.pos-products').html('<div class="col-12 text-center py-5"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div><p class="mt-2">Loading products...</p></div>');

            let data = $('.pos-product-form').serializeArray();
            console.log('getPosProducts: Form data', data);
            $.ajax({
                type: "POST",
                url: url,
                data: data,
                ...
                success: function(data) {
                    $('.pos-products').empty(); // Clear loading indicator
                ...
                error: function(xhr, status, error) {
                    ...
                    $('.pos-products').html('<div class="col-12"><div class="alert alert-danger">Error loading products. Please refresh the page.</div></div>');
                    notifyMe('error', 'Failed to load products. Please refresh the page.');
                }
            });
        }
        ...
        function getNextPosProducts() {
            if (TT.nextPosPageUrl != null) {
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        console.error('POS Next Products Error:', error);
                        $('.pos-load-more').removeClass('d-none');
                        notifyMe('error', 'Failed to load more products.');
                    }
                });
            }
        }
