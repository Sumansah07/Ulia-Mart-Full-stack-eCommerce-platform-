                                ...
                                <div class="mb-4">
                                    @php
                                        ...
                                        if (!is_null(json_decode($coupon->category_ids))) {
                                            $coupon_category_ids = json_decode($coupon->category_ids);
                                        }
                                    @endphp

                                    <label class="form-label">{{ localize('Categories') }}</label>
                                    <select id="category-select" class="form-control select2" class="w-100" data-toggle="select2"
                                        data-placeholder="{{ localize('Select Categories') }}" name="category_ids[]"
                                        multiple>
                                        <!-- Categories will be loaded via API -->
                                    </select>

                                    <!-- Store selected category IDs for JavaScript -->
                                    <input type="hidden" id="selected-category-ids" value="{{ json_encode($coupon_category_ids) }}">
                                </div>
...
@section('scripts')
    <script>
        ...
        $(document).ready(function() {
            ...
            // On form submit, ensure select2 values are submitted
            $('form').on('submit', function(e) {
                // Get selected values from select2
                const selected = $('#category-select').val() || [];
                console.log('Selected categories on submit:', selected);

                // Set hidden input to selected values as JSON (backup method)
                $('#selected-category-ids').val(JSON.stringify(selected));

                // Ensure the select has the name attribute for direct submission
                $('#category-select').attr('name', 'category_ids[]');

                console.log('Form submitting with categories:', selected);
            });
        });

        function fetchCategories() {
            // Get previously selected category IDs
            const selectedCategoryIds = JSON.parse(document.getElementById('selected-category-ids').value || '[]');
            console.log('Previously selected category IDs:', selectedCategoryIds);
            ...
            $.ajax({
                ...
                type: 'GET',
                dataType: 'json',
                data: { simple: 1 }, // Request simple format
                success: function(response) {
                    ...
                    if (response && response.success && response.data && response.data.length > 0) {
                        ...
                        // Trigger change to update select2 with selected values
                        categorySelect.trigger('change');

                        console.log('Categories loaded and selected successfully');
                    } else {
