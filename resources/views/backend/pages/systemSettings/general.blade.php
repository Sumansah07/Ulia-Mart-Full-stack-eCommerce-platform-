                    ...
                    <form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="card mb-4" id="section-1">
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    <label for="business_pan_number" class="form-label">{{ localize('Business PAN Number') }}</label>
                                    <input type="hidden" name="types[]" value="business_pan_number">
                                    <input type="text" id="business_pan_number" name="business_pan_number" class="form-control"
                                        value="{{ getSetting('business_pan_number') }}" placeholder="Enter 9-digit PAN number"
                                        maxlength="9" pattern="[0-9]{9}">
                                    <small class="text-muted">{{ localize('Enter your 9-digit business PAN number for Nepal tax registration') }}</small>
                                </div>

                                <div class="mb-3">
                                    <label for="timezone" class="form-label">{{ localize('Application Timezone') }}</label>
                                    <input type="hidden" name="types[]" value="timezone">
                                    <select id="timezone" class="form-control select2" name="timezone" data-toggle="select2">
                                        <option value="UTC" {{ env('APP_TIMEZONE', 'UTC') == 'UTC' ? 'selected' : '' }}>UTC (Coordinated Universal Time)</option>
                                        <option value="Asia/Kathmandu" {{ env('APP_TIMEZONE', 'UTC') == 'Asia/Kathmandu' ? 'selected' : '' }}>Asia/Kathmandu (Nepal Time)</option>
                                        <option value="Asia/Kolkata" {{ env('APP_TIMEZONE', 'UTC') == 'Asia/Kolkata' ? 'selected' : '' }}>Asia/Kolkata (India Standard Time)</option>
                                        <option value="Asia/Dhaka" {{ env('APP_TIMEZONE', 'UTC') == 'Asia/Dhaka' ? 'selected' : '' }}>Asia/Dhaka (Bangladesh Standard Time)</option>
                                        <option value="America/New_York" {{ env('APP_TIMEZONE', 'UTC') == 'America/New_York' ? 'selected' : '' }}>America/New_York (Eastern Time)</option>
                                        <option value="Europe/London" {{ env('APP_TIMEZONE', 'UTC') == 'Europe/London' ? 'selected' : '' }}>Europe/London (Greenwich Mean Time)</option>
                                        <option value="Asia/Tokyo" {{ env('APP_TIMEZONE', 'UTC') == 'Asia/Tokyo' ? 'selected' : '' }}>Asia/Tokyo (Japan Standard Time)</option>
                                        <option value="Australia/Sydney" {{ env('APP_TIMEZONE', 'UTC') == 'Australia/Sydney' ? 'selected' : '' }}>Australia/Sydney (Australian Eastern Time)</option>
                                    </select>
                                    <small class="text-muted">{{ localize('Set the timezone for order timestamps and system time. Nepal Time (Asia/Kathmandu) is recommended for Nepal-based businesses.') }}</small>
                                </div>
                            </div>
                        </div>
                        <!--general settings-->
                        ...
                        <div class="card mb-4" id="section-3">
                            <div class="card-body">
                                <h5 class="mb-4">{{ localize('Dashboard Logo, Favicon & Invoice Logo') }}</h5>
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    <label for="invoice_logo" class="form-label">{{ localize('Invoice Logo') }}</label>
                                    <input type="hidden" name="types[]" value="invoice_logo">
                                    <div class="tt-image-drop rounded">
                                        <span class="fw-semibold">{{ localize('Choose Invoice Logo') }}</span>
                                        <!-- choose media -->
                                        <div class="tt-product-thumb show-selected-files mt-3">
                                            <div class="avatar avatar-xl cursor-pointer choose-media"
                                                data-bs-toggle="offcanvas" data-bs-target="#offcanvasBottom"
                                                onclick="showMediaManager(this)" data-selection="single">
                                                <input type="hidden" name="invoice_logo" value="{{ getSetting('invoice_logo') }}">
                                                <div class="no-avatar rounded-circle">
                                                    <span><i data-feather="plus"></i></span>
                                                </div>
                                            </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="tt-vertical-step">
                                <ul class="list-unstyled">
                                    ...
                                    <li>
                                        <a href="#section-3">{{ localize('Dashboard Logo, Favicon & Invoice Logo') }}</a>
                                    </li>
                                    <li>
