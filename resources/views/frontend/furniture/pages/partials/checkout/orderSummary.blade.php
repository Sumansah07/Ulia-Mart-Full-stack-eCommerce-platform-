<div class="sidebar-widget py-6 px-4 bg-white rounded-2">
    ...
    <table class="sidebar-table w-100 mt-5">
        ...
        <tr>
            <td>(+) {{ localize('Tax') }}:</td>
            <td class="text-end">{{ formatPrice(getTotalTaxWithCouponDiscount($carts, getCoupon())) }}</td>
        </tr>
        ...
        @if (getCoupon() != '')
            ...
            @if (getCouponDiscountWithRestrictions($carts, getCoupon()) > 0)
                <tr>
                    <td>(-) {{ localize('Coupon Discount') }}:</td>
                    <td class="text-end">{{ formatPrice(getCouponDiscountWithRestrictions($carts, getCoupon())) }}
                    </td>
                </tr>
            @endif
