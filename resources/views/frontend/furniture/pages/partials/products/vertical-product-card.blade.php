<div class="vertical-product-card rounded-2 position-relative swiper-slide product-hover-zoom {{ isset($bgClass) ? $bgClass : '' }}">
    ...
    <div class="card-content">
        ...
        <h6 class="price">
            @include('frontend.furniture.pages.partials.products.pricing-without-tax', [
                'product' => $product,
                'onlyPrice' => true,
            ])
        </h6>
        ...
        @if ($isVariantProduct)
            <a href="javascript:void(0);" class="btn btn-outline-secondary btn-md d-block mt-4"
            ...
            <form action="" class="direct-add-to-cart-form">
                ...
                @if (!$isVariantProduct && $stock < 1)
                    <a href="javascript:void(0);"
                        class="btn btn-outline-secondary btn-md d-block mt-4 w-100">{{ localize('Out of Stock') }}</a>
                @else
                    <a href="javascript:void(0);"
                        onclick="directAddToCartFormSubmit(this)" class="btn btn-outline-secondary btn-md d-block mt-4 w-100 direct-add-to-cart-btn add-to-cart-text">{{ localize('Add to Cart') }}</a>
                @endif
            </form>
        @endif
