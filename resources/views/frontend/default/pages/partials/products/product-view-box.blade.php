        ...
        <div class="col-xl-6">
            <div class="product-info">
                ...
                <div class="product-meta-info mb-3">
                    @if($product->brand)
                        <p class="mb-1"><small class="text-muted">{{ localize('Brand') }}: <span class="fw-semibold">{{ $product->brand->name }}</span></small></p>
                    @endif
                    @if($product->categories()->count() > 0)
                        <p class="mb-1"><small class="text-muted">{{ localize('Category') }}: <span class="fw-semibold">{{ $product->categories[0]->collectLocalization('name') }}</span></small></p>
                    @endif
                    <p class="mb-1"><small class="text-muted">{{ localize('SKU') }}: <span class="fw-semibold">{{ $product->getSku() ?: $product->getCode() ?: 'N/A' }}</span></small></p>
                </div>
                <!-- End Product Info -->
