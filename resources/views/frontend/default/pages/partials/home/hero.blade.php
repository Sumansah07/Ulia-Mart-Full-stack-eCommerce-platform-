...
<div id="heroCarousel" class="carousel slide" data-bs-ride="carousel">
    ...
    <div class="carousel-inner">
        @if(count($sliders) > 0)
            ...
            @endphp
            <div class="carousel-item {{ $index == 0 ? 'active' : '' }}" style="background: {{ $colors['background'] }};">
                <div class="container position-relative">
                    <div class="row align-items-center justify-content-center">
                        <div class="col-md-6 {{ $colors['textClass'] }} text-start">
                            <div class="mb-2 small {{ $colors['textClass'] }}">{{ $slider->sub_title ?? 'UliaaMart' }}</div>
                            <h1 class="display-4 fw-bold mb-md-4 mb-2 {{ $colors['textClass'] }}">
                                {!! nl2br(e($slider->title ?? '100% Organic<br>Fruits &<br>Vegetables')) !!}
                            </h1>
                            <p class="mb-md-4 mb-2 {{ $colors['textClass'] }} d-md-block d-none">
                                {{ $slider->text ?? 'Praesent eu tellus vitae quam fermentum facilisis at sit amet mauris. Nam ac tristique sapien. In sollicitudin tristique urna, a venenatis purus luctus non.' }}
                            </p>
                            <a href="{{ $slider->link ?? route('products.index') }}" class="btn {{ $colors['buttonClass'] }} px-md-4 px-3 py-md-2 py-1 btn-sm-mobile">BROWSE PRODUCTS</a>
                        </div>
                        <div class="col-md-6 position-relative hero-image-container">
                            <img src="{{ $slider->image ? uploadedAsset($slider->image) : staticAsset('frontend/default/assets/img/products/product-1.png') }}"
                                 class="img-fluid hero-image" alt="{{ $slider->title ?? 'Organic Products' }}">
                            <div class="position-absolute top-0 end-0 {{ $colors['badgeClass'] }} text-white rounded-circle d-flex align-items-center justify-content-center discount-badge hero-badge" style="width: 60px; height: 60px; font-weight: 700;">{{ $colors['discount'] }}</div>
                        </div>
                    </div>
                </div>
            </div>
            @endforeach
        @else
            {{-- Fallback static content if no sliders configured --}}
                ...
                <div class="container position-relative">
                    ...
                    <div class="row align-items-center justify-content-center">
                        <div class="col-md-6 text-white text-start">
                            ...
                            <a href="{{ route('products.index') }}" class="btn btn-outline-light px-md-4 px-3 py-md-2 py-1 btn-sm-mobile">BROWSE PRODUCTS</a>
                        </div>
