            ...
            <div class="row align-items-center">
                ...
                <div class="col-xxl-10 col-xl-9 col-md-9 col-7">
                    <div class="gshop-navbar-right d-flex align-items-center justify-content-end position-relative">
                        ...
                        <!-- Standalone Category Button -->
                        <div class="category-btn-wrapper ms-3 d-none d-md-inline-block">
                            <div class="dropdown">
                                <button class="btn btn-success dropdown-toggle fw-bold" type="button" id="categoryDropdown" data-bs-toggle="dropdown" aria-expanded="false" style="background-color: #006633; border-color: #006633; padding: 8px 20px;">
                                    <i class="fa-solid fa-list-ul me-2"></i> Category
                                </button>
                                <ul class="dropdown-menu shadow-lg" aria-labelledby="categoryDropdown" style="z-index: 9999; max-height: 400px; overflow-y: auto; min-width: 200px; border: 1px solid #e9ecef;">
                                    @php
                                        ...
                                        if ($categories->isEmpty()) {
                                            $categories = \App\Helpers\CategoryHelper::getHardcodedCategories();
                                        }
                                    @endphp

                                    <li><h6 class="dropdown-header fw-bold text-success">{{ localize('Categories') }}</h6></li>
                                    @foreach($categories as $category)
                                        <li><a class="dropdown-item py-2" href="{{ route('products.index') }}?category_id={{ $category->id }}">{{ $category->name }}</a></li>
                                    @endforeach
                                </ul>
                            </div>
                        </div>
                        ...
                        <div
                            ...
                            <div class="gshop-header-category dropdown ms-3">
                                <button type="button" class="header-icon" data-bs-toggle="dropdown" aria-expanded="false">
                                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M2 4H16" stroke="#5D6374" stroke-width="2" stroke-linecap="round"/>
                                        <path d="M2 9H16" stroke="#5D6374" stroke-width="2" stroke-linecap="round"/>
                                        <path d="M2 14H16" stroke="#5D6374" stroke-width="2" stroke-linecap="round"/>
                                    </svg>
                                    <span class="d-none d-lg-inline-block ms-1 fw-medium">Category</span>
                                </button>
                                <div class="dropdown-menu dropdown-menu-end shadow-lg border-0" style="z-index: 9999; max-height: 400px; overflow-y: auto; min-width: 200px;">
                                    @php
                                        $categories = \App\Models\Category::where(function($query) {
                                                $query->whereNull('parent_id')
                                                    ->orWhere('parent_id', 0)
                                    ...
                                    <h6 class="dropdown-header fw-bold">{{ localize('Categories') }}</h6>
                                    @foreach($categories as $category)
                                        <a class="dropdown-item py-2" href="{{ route('products.index') }}?category_id={{ $category->id }}">
                                            {{ $category->name }}
                                        </a>
                                    @endforeach
                                </div>
                                ...
                                <div class="cart-box-wrapper">
                                    <div class="apt_cart_box theme-scrollbar">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between mt-3">
                                                    ...
                                                    foreach ($carts as $cart) {
                                                        try {
                                                            if (isset($cart->product_variation) && isset($cart->product_variation->product)) {
                                                                ...
                                                                $cartSidebarSubtotal += $discountedPrice * $cart->qty;
                                                            }
                                                        } catch (\Exception $e) {
                                            ...
                                            @endphp
                                            <span class="fw-semibold text-secondary sub-total-price">{{ formatPrice($cartSidebarSubtotal) }}</span>
                                        </div>
...
<style>
    ...
    .gshop-header-category .dropdown-menu {
        display: none;
    }

    .gshop-header-category.show .dropdown-menu {
        display: block;
    }

    .gshop-header-category .dropdown-item:hover {
        background-color: #f8f9fa;
        color: #006633;
    }

    .gshop-header-category .dropdown-header {
        color: #006633;
        font-size: 14px;
        padding: 8px 16px;
    ...
    .gshop-header-icons .dropdown-menu {
        position: absolute !important;
        transform: none !important;
    }

    /* Standalone Category Button Styles */
    .category-btn-wrapper .dropdown-menu {
        position: absolute !important;
        transform: none !important;
    }

    .category-btn-wrapper .dropdown-item:hover {
        background-color: #f8f9fa;
        color: #006633;
    }

    .category-btn-wrapper .dropdown-header {
        color: #006633;
        font-size: 14px;
        padding: 8px 16px;
        border-bottom: 1px solid #e9ecef;
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Make sure the category dropdown is visible
        const categoryDropdown = document.querySelector('.gshop-header-category .dropdown-toggle');
        if (categoryDropdown) {
